...
<div class="flex items-center lg:gap-x-3">
    @if ($reply->author()->isModerator() || $reply->author()->isAdmin())
        <span class="text-sm text-purple-600 border border-lio-200 rounded py-1.5 px-3 leading-none">
            moderator
        </span>
    @endif
    ...
    @can(App\Policies\ThreadPolicy::UPDATE, $thread)
        @if ($thread->isSolutionReply($reply))
            <button
                class="flex items-center font-medium text-purple-600 hover:text-gray-300"
        ...
        @else
            <button
                class="flex items-center font-medium text-gray-300 hover:text-purple-600"
    ...
    @else
        @if ($thread->isSolutionReply($reply))
            <span class="flex items-center font-medium text-purple-600">
                <x-heroicon-o-check-badge class="w-6 h-6" />
                <span>Solution</span>
            </span>
        @endif
